...
import modalFactory, { defaultModal, ModalEvents, showAlertModal } from '../theme/global/modal';
...
export default function (context) {
    ...
    $('body').on('click', '[data-papathemes-cart-item-add]', (event) => {
        ...
        const productId = getURLParam('product_id', event.currentTarget.href);
        if (productId === 0) {
            return;
        }
        ...
        if (qty && qty.length > 0) {
            ...
            } else if ($(event.currentTarget).is('[data-check-qty]')) {
                showAlertModal(context.ajaxAddToCartEnterQty || 'Please enter quantity');
                return;
            }
        }
