    ...
    schema.forEach((group, i) => {
        ...
        if (!group.settings || !(group.settings instanceof Array)) {
            ...
            group.settings.forEach((item, j) => {
                if (!item.type || ['heading', 'color', 'select', 'font', 'text', 'input', 'checkbox', 'imageDimension', 'paragraph', 'optimizedCheckout-image'].indexOf(item.type) === -1) {
                ...
                if (item.type === 'color' || item.type === 'font' || item.type === 'text' || item.type === 'input' || item.type === 'imageDimension' || item.type === 'optimizedCheckout-image') {
                    valueType = 'string';
